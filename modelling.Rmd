...
model_fit1 <- modeltime_wfs_fit(.wfsets = model_work1, 
                            .split_prop = 0.8, 
                            .serie=df)
fit1 <- prophet_catboostAF %>% fit(base, data = df_train)
...
modelo1 <- prophet_xgboost_logMF %>%
  fit(rate~Time, data = df_train)
residuos <- as.data.frame(NULL)
for(i in 1:nrow(model_fit1)){
  if(i == 1){
  residuos <- model_fit1$`.fit_model`[[i]]$.model[[1]] %>%
  modeltime_calibrate(new_data = df_train) %>%
  modeltime_residuals() %>%
  modeltime_residuals_test()
  }else{
    residuos[nrow(residuos) +1, ] <- model_fit1$`.fit_model`[[i]]$.model[[1]] %>%
  modeltime_calibrate(new_data = df_train) %>%
  modeltime_residuals() %>%
  modeltime_residuals_test()
  }
